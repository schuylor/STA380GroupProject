...
# Load Data
```{r}
lizard_data <- read.csv("submissions/BasicImplementation/bootstrap.R",
 fileEncoding = "latin1")
...
boot_mean_snout_len_dist <- data.frame(diffs = boot_mean_snout_len)
#Plot bootstrap Distribution Histogram
ggplot(boot_mean_snout_len_dist, aes(x = diffs))  +
...
```{r}
# Convert to dataframe
boot_mean_body_temp_dist <- data.frame(diffs = boot_mean_body_temp)
#Plot bootstrap Distribution Histogram
ggplot(boot_mean_snout_len_dist, aes(x = diffs))  +
