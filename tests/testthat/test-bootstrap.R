...
test_that("mean vs median option returns valid outputs", {
  ...
  expect_type(out_mean, "double"); expect_type(out_med, "double")
  expect_length(out_mean, 200); expect_length(out_med, 200)
  expect_true(all(is.finite(out_mean))); expect_true(all(is.finite(out_med)))
})
...
test_that("function runs on the project dataset (smoke test)", {
  ...
  len_col <- grep("snout|vent|svl|length", colnames(df), ignore.case = TRUE, value = TRUE)[1]

  expect_true(!is.na(sex_col) && nzchar(sex_col))
  expect_true(!is.na(len_col) && nzchar(len_col))
